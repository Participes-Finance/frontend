...
export default function useDataStore() {
  ...
  async function poll() {
    ...
    masterChefV2.value = new Contract(
      MASTERCHEFV2_ADDRESSES?.get(chainId as string) as string,
      JSON.stringify(MasterChefV2ABI),
      signer.value
    );
    
    poolLength.value = ((await masterChefV2.value.poolLength()) as BigNumber).toNumber();

    veContract.value = new Contract(
      VE_ADDRESSES?.get(chainId as string) as string,
      JSON.stringify(veABI),
      signer.value
    );
    
    distContract.value = new Contract(
      DIST_ADDRESSES?.get(chainId as string) as string,
      JSON.stringify(distABI),
      signer.value
    );
